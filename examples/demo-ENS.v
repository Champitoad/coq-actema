...
Lemma swap_args (h:A -> B -> C) : B -> A -> C.
Proof.
  actema.
Qed.

Lemma curry (h: A/\B -> C) : A -> B -> C.
Proof.
  actema.
Qed.

Lemma uncurry (h: A -> B -> C) : A /\ B -> C.
Proof.
  actema.
Qed.


Lemma sym_eq_nat : forall x y : nat, x = y -> y = x.
actema.
Qed.

Lemma trans_eq (x:nat) y z (h1 : x = y)(h2 : y = z) : x = z.
actema.
Qed.
...
Parameter f : X -> Y.
Parameter g : Y -> X.
Lemma inv_d (inv : forall y, f (g y) = y)(surj : forall x, exists y,  g y = x) :
              forall x,  g (f x) = x.
actema_force.
Qed.
