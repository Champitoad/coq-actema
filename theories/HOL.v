                       ...
                       if b then
                        match (check_curry _ _ _ s f (ppconcat _ _ gi1 hi))
                         ...
                         | Some r =>
                           ...
                           (*
                 let f' :=
                   if b then (switch_inst1 f)
                   else f
                 in
                 match f' with
                 |  (insti si ni1 ni2 fi) =>
                      match (check_list _ nh ni1 hi),
                        (check_list _ ng' ni2 gi)
                      with
                      |Some hi2, Some gi2 =>
                         match (check_nat _ si s (fi hi2 gi2))
                         with
                         | Some r =>
                             b3 rw l b ist
                                nh hyp hi
...
(*
              ...
              | insti s1 ni1 ni2 fr =>
                  ...
                  match (check_list _ nh1 ni1 hi1),
                        (check_list _ ng ni2 gi)
                      with
                      |Some hi2, Some gi2 =>
                         match (check_nat _ s1 s (fr hi2 gi2))
                         with
                         | Some r =>
                             b3 rw l b ist
                                _
   ...
   |  true  =>
       ...
       | orr n2' B h2', n2, i2  =>
           orr3 (B i2)
                (f3 rw l b ist n1 h1 i1 n2' h2' i2) 
       | impl n2' h2' B, n2, i2  =>
           impl3 (b3 rw l (negb b) ist n1 h1 i1 n2' h2' i2)
                 (B i2)
       | cNot n2' h2', n2, i2  =>
           not3
      ...
      | impr n2' B h2', n2, i2  =>
       ...
       | fa n2' s2 nv h2', n2, i2  =>
          ...
          | cons (Some (insti s' n f)) ist =>
              if b then
                match  (check_curry _ _ _ s2 f (ppconcat _ _ i1 i2)) with
                | None => top3
                | Some r =>
                    f3 rw l b ist n1 h1 i1 _ h2' (r, i2)
                end
              else
                match  (check_curry _ _ _ s2 f (ppconcat _ _ i2 i1)) with
                | None => top3
                | Some r =>
                    f3 rw l b ist n1 h1 i1 _ h2' (r, i2)
                end
            (*
              ...
              match f' with
              | insti sr nr1 nr2 fr =>
                  match (check_list _ n1 nr1 i1),  (check_list _ n2 nr2 i2)
                  with
                  | Some ir1, Some ir2 =>
                      match check_nat _ sr s2 (fr ir1 ir2)
                      with
                      | Some r =>
                          f3 rw l b ist n1 h1 i1 _ h2' (r, i2)
                      | _ => top3
...
move => [n hn]; eauto])).
 ...
 (move: pv;
    case: ist => [//=|[[sr nr1 nr2 gr]|] ist]//=;
 case check_list => //= p2; case check_list => //= p1; case check_nat => //= s'; eauto;
move => [n hn]; eauto).
Qed.
...
Goal  (forall x y, x = true -> y = 4) -> False.
  intro h.
  ltac:(dyn_inst_hyp (cons false (cons false nil))  h h' 8).
Abort.
...
intro h.
dyn_inst_hyp_nd  (cons false nil) h False.
by apply h.
Abort. 
...
Parameter ggg : nat -> nat -> bool.
Eval  compute in (ty_curry (sl ts) ([:: 1] ++ [:: 1]) 2).
Check ltac:(let r :=
              reify_uncurry ts ( nat -> nat -> bool) in exact r).



Check ltac:(let r :=
              dress1 ts ggg in exact r).

Ltac dress ts l :=
  match l with
  | cons (Some (mDYN _ ?f)) ?l' => 
      let fi := dress1 ts f in 
      let lr := dress ts l' in 
      constr:(cons (Some fi) lr) 
  | cons (Some ?f) ?l' => 
      let fi := dress1 ts f in 
      let lr := dress ts l' in 
      constr:(cons (Some fi) lr) 
  ...
  match o with
    | coerce _ (@nil nat) ?hc _ =>
       ...
       try exact tt];
         ...
         in
         rewrite   /b3 /o3_norm /trl3  /coerce /xor     /tr3 /eqnqtdec /nat_rec /nat_rect /eq_rect_r /eq_rect  /eq_ind_r /eq_ind  /eq_sym /list_rect /ts  /wsort /sl;
         let tt :=
           match goal with
           | |- ?tt => tt
           end
...
Ltac back h hp gp t i := depiot i.
(*
let th := type of h in
  let ts1 := mkSign hp th in 
  match goal with
  | |- ?tg =>
      let ts := mkSignR ts1 gp  (fun XX : (forall TT:Type, TT) => tg) in
...
Goal exists f : nat -> nat -> nat , forall z, (f (S z) 4) = (S (S z)).
Print ex_intro.  
eapply (ex_intro).
move => z.
exact (refl_equal ((fun x y : nat => _) (S z) 4)).
Save toto.
Print toto.
...
Ltac rew_dnd_o ts' h hp gp gp' t i :=
 ...
 let gc :=
  ...
  in
  rewrite   /b3 /trl3 /o3_norm /coerce  /xor /tr3 /eqnqtdec /nat_rec /nat_rect  /list_rect
...
Ltac rew_dnd  h hp gp gp' t i :=
 (* (rew_dnd_oo h hp gp gp' t i) + *)
    (rew_dnd_rev  h gp gp' hp t i).
...
(*  end *) ;
  ...
  rewrite /f3 /trl3 /o3_norm  /coerce /xor  /tr3 /eqnqtdec /nat_rec /nat_rect /eq_rect_r /eq_rect  /eq_ind_r /eq_ind  /eq_sym /list_rect /ts  /wsort /sl in h3;
...
Ltac forward  h1' h2' h3 hp1 hp2 t i :=
  ...
  let ts := mkSignR ts1 hp2  (fun XX : (forall TT:Type, TT) => h2') in
