...
let check_action (action : 'a option) (pred : 'a option -> bool) : unit =
  if pred action
  then ()
...
let link_backward hyp hyp_sub concl concl_sub (hlpred : 'a Link.Pred.t) :
    'a option =
  (* Make a test proof. *)
...
let test_pol_0 () =
  let open Term in
  let hlpred = Link.Pred.opposite_pol_formulas in
  let hyp =
    mkArrow
  ...
  in
  let action = link_backward hyp [ 0; 1 ] concl [ 1; 2; 2; 1 ] hlpred in
  check_action action Option.is_some
...
let test_pol_1 () =
  ...
  in
  let action = link_backward hyp [ 1; 1 ] concl [ 1; 2; 2; 1 ] hlpred in
  check_action action Option.is_some
...
let test_eq_0 () =
  let open Term in
  let hlpred = Link.Pred.neg_eq_operand in
  let hyp =
    mkArrow
  ...
  in
  let action = link_backward hyp [ 0; 1; 1; 2; 1; 2 ] concl [ 1 ] hlpred in
  check_action action @@ function Some `Left -> true | _ -> false
...
let test_eq_1 () =
  ...
  in
  let action = link_backward hyp [ 1 ] concl [ 1; 1; 0; 3 ] hlpred in
  check_action action @@ function Some `Right -> true | _ -> false
...
let check_unif (action : Link.unif_data option) (t1, sub1) (t2, sub2) =
  ...
  | Some unif ->
      Term.alpha_equiv
        (Unif.apply unif.subst @@ subterm t1 sub1 unif.fvars_1)
        (Unif.apply unif.subst @@ subterm t2 sub2 unif.fvars_2)
  | _ -> false
...
let test_unif_0 () =
  ...
  let sub2 = [] in
  let action = link_forward hyp1 sub1 hyp2 sub2 hlpred in
  check_unif action (hyp1, sub1) (hyp2, sub2)
...
let test_unif_1 () =
  ...
  let action = link_backward hyp hyp_sub concl concl_sub hlpred in
