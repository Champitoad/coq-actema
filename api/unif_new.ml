...
type sitem = SRigid | SFlex | SBound of Term.t | SAlias of FVarId.t
[@@deriving show]
...
type subst =
  { (* We use a *mutable* map from variables to sitems, to allow for path compression. *)
    tbl : sitem FVarId.Hashtbl.t [@printer print_table]
  }
[@@deriving show]

(** Find the representative of a variable up to aliasing.
    This assumes [v1] and [v2] are either [SAlias] or [SFlex] (or [SRigid] ?). *)
let find_repr subst v = failwith "todo"

(** [alias subst v1 v2] assumes [v1] is either [SAlias] or [SFlex]. 
    However [v2] can be anything. *)
let alias subst v1 v2 = failwith "todo"
