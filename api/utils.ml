...
module LEnv = struct
  let exists lenv x = List.exists (fun (y, _) -> x = y) lenv
  let enter lenv x ty = (x, ty) :: lenv
end
...
module Funs = struct
  let get (env : env) (name : name) = List.assoc_opt name env.env_fun
end
...
let rec einfer (env : env) (e : expr) : type_ =
  ...
  | `EFun (f, args) -> (
      ...
      | Some (fargs, fres) ->
          ...
          if not (List.for_all2 t_equal fargs args) then raise TypingError;
          fres)
...
let subexpr (e : expr) (p : int list) =
  match subterm (`E e) p with `E e -> e | _ -> raise (InvalidSubExprPath p)

type item = [ `C of form | `H of uid * hyp | `V of name * bvar ]
...
let of_ipath (goal : goal) (p : ipath) : goal * item * (int list * term) =
  ...
  let item, t_item =
    ...
    | `Concl, _ ->
        ...
        (`C f, `F f)
    | `Hyp, hd ->
        let ({ h_form = hf; _ } as hyd) = byid goal.g_hyps hd in
        (`H (hd, hyd), `F hf)
    | `Var part, hd ->
        let ((_, body) as def) =
          match Vars.byid goal.g_env hd with Some v -> v | None -> raise exn
        in
        let expr : Fo_t.expr =
          match part with
          | `Head -> `EVar hd
          | `Body -> ( match body with Some b -> b | None -> raise exn)
        in
        (`V (hd, def), `E expr)
  in
...
let string_of_expr e = Bi_io.view ~unhash:biniou_unhash_dict (Logic_b.string_of_expr e)
let string_of_form f = Bi_io.view ~unhash:biniou_unhash_dict (Logic_b.string_of_form f)
let string_of_term t = Bi_io.view ~unhash:biniou_unhash_dict (Logic_b.string_of_term t)
let string_of_env env = Bi_io.view ~unhash:biniou_unhash_dict (Logic_b.string_of_env env)
let string_of_goal goal = Bi_io.view ~unhash:biniou_unhash_dict (Logic_b.string_of_goal goal)
let string_of_goals goals = Bi_io.view ~unhash:biniou_unhash_dict (Logic_b.string_of_goals goals)
let string_of_itrace itr = Bi_io.view ~unhash:biniou_unhash_dict (Logic_b.string_of_itrace itr)
let string_of_action a = Bi_io.view ~unhash:biniou_unhash_dict (Logic_b.string_of_action a)
