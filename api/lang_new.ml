...
module Term = struct
  ...
  type t =
    ...
    | Lambda of (cdata[@opaque]) * Name.t option * t * t
    | Prod of (cdata[@opaque]) * Name.t option * t * t
  [@@deriving show]
...
module Context = struct
  type entry = { name : Name.t option; type_ : Term.t } [@@deriving show]
  ...
  type t =
    ...
    ; (* An offset used to generate fresh free variable identifiers. *)
      fvar_offset : int
    }
  [@@deriving show]

  (** The empty typing context. *)
  let empty = { map = FVarId.Map.empty; fvar_offset = 0 }

  let fresh_fvar ctx =
    (* A context is in charge of generating a free variable,
       so it needs access to the internal representation of FVarId. *)
    ( FVarId.unsafe_from_int ctx.fvar_offset
    , { ctx with fvar_offset = ctx.fvar_offset + 1 } )

  let size ctx = FVarId.Map.cardinal ctx.map
  let add fvar entry ctx = { ctx with map = FVarId.Map.add fvar entry ctx.map }

  let add_fresh entry ctx =
    let fvar, ctx = fresh_fvar ctx in
    (fvar, add fvar entry ctx)

  let find fvar ctx = FVarId.Map.find_opt fvar ctx.map
  let remove fvar ctx = { ctx with map = FVarId.Map.remove fvar ctx.map }
end
...
module Env = struct
  ...
  let test_env =
    ...
    let constants =
      ...
      ; ( Constants.eq
        , mkProd (Some (Name.make "A")) mkType
...
module TermUtils = struct
  let rec subterm_rec ctx t sub : Context.t * Term.t =
    ...
    | idx :: sub -> begin
        ...
        | (Lambda (_, x, ty, body) | Prod (_, x, ty, body)) when idx = 1 ->
            ...
            let fvar, new_ctx =
              Context.add_fresh Context.{ name = x; type_ = ty } ctx
            in
  ...
  let rec check_rec env (ctx : Context.t) t : Term.t =
    ...
    | Lambda (_, x, ty, body) ->
        ...
        (* Check the body. *)
        let fvar, new_ctx = Context.add_fresh { name = x; type_ = ty } ctx in
    ...
    | Prod (_, x, ty, body) -> begin
        ...
        (* Type check the body. *)
        let fvar, ctx = Context.fresh_fvar ctx in
        let body_ty =
          check_rec env (Context.add fvar { name = x; type_ = ty } ctx)
