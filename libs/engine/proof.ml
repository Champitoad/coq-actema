...
let elim ?clear (h : Handle.t) ((pr, id) : targ) =
  ...
  (* [direct_subform_pol (p, f) i] returns the [i]th direct subformula of [f] together
  ...
  let dnd_actions src dsts (proof : Proof.proof) =
    ...
    let for_destination dst =
      ...
      let targets =
        let open Monad.List in
        subs f_src >>= fun sub_scrutinee ->
        let pol_scrutinee, scrutinee = subform_pol (pol_src, f_src) sub_scrutinee in
        search_match g_pregoal.g_env pol_scrutinee scrutinee pol_dst f_dst |>
        List.map (fun (sub_target, s) ->
